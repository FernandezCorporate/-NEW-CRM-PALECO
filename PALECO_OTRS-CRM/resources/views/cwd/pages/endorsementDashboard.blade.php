...
@section('title', 'Endorsement Requests')
...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Endorsement Requests</h1>
            <p class="text-sm text-gray-500 mt-1">Review and act on endorsements submitted by supervisors on active tickets</p>
        </div>
    ...
    <nav class="record-tabs" aria-label="Filter endorsement requests">
    ...
    <div class="mb-6">
        <form action="{{ request()->url() }}" method="GET" class="flex flex-col lg:flex-row gap-4 items-center w-full">
            ...
            <div class="relative w-full flex-grow">
                ...
                <input type="text" name="search" placeholder="Search endorsements by Ticket ID..." value="{{ request('search') }}" class="w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-md text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-[#008f5d] focus:ring-1 focus:ring-[#008f5d]">
            </div>
                    ...
                    @forelse ($endorsements as $endorsement)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 align-top">
                                <div class="font-bold text-[#008f5d] text-sm">{{ $endorsement->ticket->ticket_number ?? 'N/A' }}</div>
                                <div class="text-[11px] text-gray-500 mt-0.5 line-clamp-1">
                                    {{ $endorsement->ticket->subject }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 align-top">
                                <div class="text-sm text-gray-800 font-medium flex items-center gap-2">
                                    ...
                                    {{ $endorsement->creator->full_name ?? 'N/A' }}
                                </div>
                                <div class="text-[11px] text-gray-400 mt-1 ml-6">{{ $endorsement->creator->department->dept_name ?? 'Field Team' }}</div>
                            </td>
                            ...
                            <!-- Requested Date -->
                            <td class="px-6 py-4 align-top">
                                <div class="text-sm text-gray-800">{{ $endorsement->created_at->format('Y-m-d H:i') }}</div>
                            </td>
                            ...
                            <!-- Action -->
                            <td class="px-6 py-4 align-top text-center">
                                <a href="{{ route('cwd.endorsements.show', $endorsement) }}" class="inline-flex items-center justify-center px-3 py-1.5 border border-gray-300 shadow-sm text-[11px] font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#008f5d] transition-colors">
                                    Review
                                </a>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center border-2 border-dashed border-gray-200 rounded-lg">
                                ...
                                <h3 class="text-sm font-bold text-gray-900">No endorsement requests</h3>
                                <p class="text-xs text-gray-500 mt-1">There are currently no endorsements matching your criteria.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @if($endorsements->hasPages())
        <div class="mt-4">
            {{ $endorsements->onEachSide(1)->links() }}
        </div>
    @endif
