...
@section('content')
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-slate-800">Create New Department</h1>
        <p class="text-sm text-slate-500 mt-1">Add a new department to the cooperative structure.</p>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.departments.store') }}" method="POST">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea id="dept_desc" name="dept_desc" rows="4" 
                        class="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm transition-colors focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10">{{ old('dept_desc') }}</textarea>
            ...
            <div class="bg-slate-50 px-6 py-4 border-t border-slate-200 flex items-center justify-end gap-3">
                <a href="{{ route('admin.departments') }}" class="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-800 hover:bg-slate-200 rounded-lg transition-colors">
                    Cancel
                </a>
