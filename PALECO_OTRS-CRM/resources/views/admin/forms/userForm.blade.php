        ...
        <form method="POST" action="{{ isset($user) ? route('admin.users.update', ['user' => $user]) : route('admin.users.store') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-6">
                    ...
                    <select name="department_id" class="w-full px-3.5 py-2.5 border rounded-lg shadow-sm text-sm focus:outline-none transition-colors bg-white cursor-pointer
                        ...
                        @foreach ($depts as $dept)
                            <option value="{{ $dept->id }}" {{ old('department_id', optional($user)->department_id) == $dept->id ? 'selected' : '' }}>
                                {{ $dept->dept_name }}
                            </option>
                        @endforeach
                    </select>
